...
TEST_CASE("Basics", "[basics]") { // NOLINT
  ...
  SECTION("Addition") {
    CHECK(get<long>(eval("Plus"_(5, 4))) == 9); // NOLINT
    CHECK(get<long>(eval("Plus"_(5, 2, 2))) == 9);
    CHECK(get<long>(eval("Plus"_(5, 2, 2))) == 9);
    CHECK(get<long>(eval("Plus"_("Plus"_(2, 3), 2, 2))) == 9);
    CHECK(get<long>(eval("Plus"_("Plus"_(3, 2), 2, 2))) == 9);
  }
  ...
  SECTION("Symbols") {
    ...
    CHECK(get<long>(expression.getArguments().at(0)) == 9);
  ...
  SECTION("Relational (with multiple column types)") {
    ...
    REQUIRE(get<long>(eval("Length"_("Select"_("Customer"_, "Function"_(true))))) == 0);
    auto const& emptyTable = eval("Select"_("Customer"_, "Function"_(true)));
    CHECK(get<long>(eval("Length"_(emptyTable))) == 0);
    ...
    CHECK(get<long>(eval("Length"_(fullTable))) == 3);
    ...
    SECTION("Selection") {
      ...
      CHECK(get<long>(eval("Length"_(sam))) == 1);
      auto const& samRow = eval("Extract"_(sam, 1));
      CHECK(get<long>(eval("Length"_(samRow))) == 5);
      ...
      CHECK(get<long>(eval("Length"_(none))) == 0);
      auto const& all = eval("Select"_("Customer"_, "Function"_(true)));
      CHECK(get<long>(eval("Length"_(all))) == 3);
    ...
    SECTION("Projection") {
      ...
      INFO(fullnames);
      CHECK(get<long>(eval("Length"_(fullnames))) == 3);
    ...
    SECTION("Aggregation") {
      ...
      CHECK(get<long>(eval("Extract"_("Extract"_(countRows, 1), 1))) == 3);
      CHECK(get<long>(eval("Extract"_(
...
TEST_CASE("Arrays", "[arrays]") { // NOLINT
  ...
  SECTION("ArrowArrays") {
    CHECK(get<long>(eval("Extract"_(arrayPtrExpr, 1))) == 10);
    CHECK(get<long>(eval("Extract"_(arrayPtrExpr, 2))) == 20);
    CHECK(get<long>(eval("Extract"_(arrayPtrExpr, 3))) == 30);
    CHECK(get<long>(eval("Extract"_(arrayPtrExpr, 4))) == 40);
    CHECK(get<long>(eval("Extract"_(arrayPtrExpr, 5))) == 50);
  ...
  SECTION("Plus") {
    compareColumn("Project"_("Thingy"_, "As"_("Result"_, "Plus"_("Value"_, "Value"_))),
                  vector<long>{20, 40, 60, 80, 100}); // NOLINT(readability-magic-numbers)
    compareColumn("Project"_("Thingy"_, "As"_("Result"_, "Plus"_("Value"_, 1))),
                  vector<long>{11, 21, 31, 41, 51}); // NOLINT(readability-magic-numbers)
  }
