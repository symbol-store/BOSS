...
TEST_CASE("Expression Serialization With Spans") {
  ...
  auto const dataSetSize = 20;
  std::vector<int8_t> vec1(dataSetSize);
  std::vector<int8_t> vec2(dataSetSize);
  std::iota(vec1.begin(), vec1.end(), 0);
  ...
  spanArgs.push_back(std::move(boss::Span<int8_t>(vector(vec4))));
